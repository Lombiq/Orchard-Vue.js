...
5. Before you start, add this to your project file so the *.vue* files are recognized as embedded views:
...
What you need to know to write your own *.vue* file:
...
- If your component has child components, include the *.vue* extension when importing them.

For example if you have the file *My.Module/Assets/Scripts/VueComponents/my-article.vue*:
...
You can include the `<vue-component area="My.Module name="my-article">` tag helper in your code. This will add Vue and *My.Module/wwwroot/vue/my-article.js* to the resource manager (using the `vue-component-{name}` resource) as well as the `VueComponent-MyArticle` shape (the SFC's kebab-case name is converted into PascalCase). Include `depends-on="vue-component-my-article"` in your app's `<script>` element to ensure correct load order.
