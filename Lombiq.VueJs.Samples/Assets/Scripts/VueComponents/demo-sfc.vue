<template>
    <!-- This is the template of the component. If you have IDE plugin for Vue.js, this format affords you first-class
         Vue.js coding support. -->
    <div class="DemoSfc__wrapper">
        <select class="DemoSfc__select" @change="$emit('update:model-value', parseInt($event.target.value))">
            <!-- Note the bound "key" property - it must be unique and you should always include it for performance
                 reasons. Also the "value" binding is not strictly necessary but can be useful for communicating value
                 with external libraries, e.g. with other JS scripts or during UI testing. -->
        ...
        <!-- Here you can see a non-standard expression. The "[[ ... ]]" is unique to this OC module: it performs
             localization via IStringLocalizer at runtime. -->
        ...
        <!-- You can use different converters inside the "[[ ... ]]" to achieve different results. For example the
             "[[{ ... }]]" expression runs the contents through IHtmlLocalizer, and treats it like encoded HTML. -->
...
<script>
// You can include other components and JS files. The export must be ESM style like below (export default { ... }) so
// the imports should be too. When you import an SFC make sure to include the .vue file extension to reliably tell it
// apart from plain JS modules.
...
export default {
    ...
    // Computed properties are like getters, but they are cached and reactive. For example this one is only re-evaluated
    // if self.modelValue changes which triggers an update.
    computed: {
        repeaterData(self) {
    ...
    // You can include multiple child components. During build the exported SFC is registered as a global component, but
    // its children remain private, composed using the "components" property.
    components: {
        DemoRepeater,
    },
};
